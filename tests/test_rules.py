...
SCENARIOS_DIR = Path(__file__).parent / "scenarios" / "rules"

_SCENARIOS = sorted(SCENARIOS_DIR.glob("PW*.py"))


def _parse_expected_errors(scenario: Path) -> set[tuple[int, str]]:
    """Extract ``(line_number, code)`` pairs from ``# error: PWXXX`` comments."""
    errors: set[tuple[int, str]] = set()
    for i, line in enumerate(scenario.read_text().splitlines(), start=1):
...
# ---------------------------------------------------------------------------
# Scenario-based rule violation tests
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("scenario", _SCENARIOS, ids=lambda p: p.stem)
class TestRuleViolation:
    """Verify rule detection using scenario files.

    Each scenario in ``tests/scenarios/rules/`` contains handler code
    annotated with ``# error: PWXXX`` comments marking expected
    violations, plus OK code that should not trigger any violations.
    """

    def test_expected_violations_detected(self, scenario: Path) -> None:
        """All lines marked with '# error:' produce the expected violation."""
        expected = _parse_expected_errors(scenario)
        with patch("pythaw.finder._git_ls_files", return_value=None):
            violations = check(scenario, Config())
        actual = {(v.line, v.code) for v in violations}
        assert actual == expected

    def test_no_false_positives(self, scenario: Path) -> None:
        """Lines without '# error:' do not produce violations."""
        expected_lines = {line for line, _ in _parse_expected_errors(scenario)}
        with patch("pythaw.finder._git_ls_files", return_value=None):
            violations = check(scenario, Config())
